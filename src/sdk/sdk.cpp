...
{
	std::uint64_t name_container = memory->read<std::uint64_t>(
		this->address + Offsets::Instance::NameContainer
	);
	...
	return memory->read_string(
		name_container + Offsets::Instance::Name
	);
}

std::string rbx::nameable_t::get_class_name() const
{
	std::uint64_t class_descriptor = memory->read<std::uint64_t>(
	...
	std::uint64_t class_name_ptr = memory->read<std::uint64_t>(
