...
void rage::orbit::run()
{
	...
	while (true) {
		...
		cache::entity_t target{};
		{
			...
			// orbit_type: 0 = aimbot target, 1 = silent aim target, 2 = playerlist selected target
			...
			else if (settings::movement::orbit::orbit_type == 2)
			{
				// orbit the player selected in the playerlist target
				if (!settings::rage::playerlist::target_name.empty())
				{
					for (const auto& entity : cache::cached_players)
					{
						if (entity.name == settings::rage::playerlist::target_name)
						{
							target = entity;
							break;
						}
