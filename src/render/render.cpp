...
bool render_t::create_imgui()
{
	...
	font_cfg_tahoma.FontLoaderFlags = ImGuiFreeTypeLoaderFlags_ForceAutoHint;
	font_cfg_tahoma.PixelSnapH = false;
	font_cfg_tahoma.OversampleH = 3;
	font_cfg_tahoma.OversampleV = 3;
	font_cfg_tahoma.RasterizerMultiply = 1.0f;
	font_cfg_tahoma.GlyphOffset = ImVec2(0, 0);
...
void render_t::end_render()
{
	...
	// FPS cap for overlay (0 = uncapped)
	static auto last_frame = std::chrono::steady_clock::now();
	int cap = 0;
	try { cap = settings::visuals::overlay_fps_cap; } catch (...) { cap = 0; }
	if (cap > 0) {
		auto now = std::chrono::steady_clock::now();
		int target_ms = 1000 / cap;
		int elapsed = static_cast<int>(std::chrono::duration_cast<std::chrono::milliseconds>(now - last_frame).count());
