...
#ifdef _WIN32
    // Try to download offsets.json for the given version from imtheo.lol
    // Returns true on success (file saved to 'path'), false on failure
    inline bool TryDownloadOffsetsFromImTheo(const std::string& version, const std::string& path = "offsets.json") {
        if (version.empty()) return false;
        std::string url = std::string("https://offsets.imtheo.lol/") + version + "/offsets.json";

        printf("Attempting to download offsets from %s\n", url.c_str());

        // Use URLDownloadToFileA from Urlmon
        // Include urlmon and link via pragma
#ifndef _WIN32
        (void)version; (void)path; return false;
#else
        // Delay-include urlmon to avoid requiring the header for non-Windows builds
#include <urlmon.h>
#pragma comment(lib, "urlmon")
        HRESULT hr = URLDownloadToFileA(NULL, url.c_str(), path.c_str(), 0, NULL);
        if (SUCCEEDED(hr)) {
            printf("Downloaded offsets to %s\n", path.c_str());
            return true;
        } else {
            printf("Failed to download offsets (hr=0x%08x)\n", static_cast<unsigned int>(hr));
            return false;
        }
#endif
    }
