...
bool Menu::Initialize(HWND hWnd, ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
    ...
    // More modern rounded style with pale forest green accents
    ...
    style.Colors[ImGuiCol_WindowBg] = ImAdd::HexToColorVec4(0x0f2418);
    style.Colors[ImGuiCol_ChildBg] = ImAdd::HexToColorVec4(0x0f2418);
    style.Colors[ImGuiCol_PopupBg] = ImAdd::HexToColorVec4(0x0c1e15);
    ...
    style.Colors[ImGuiCol_Header] = ImAdd::HexToColorVec4(0x2E8B57);
    style.Colors[ImGuiCol_HeaderHovered] = ImAdd::HexToColorVec4(0x3AA56B);
    style.Colors[ImGuiCol_HeaderActive] = ImAdd::HexToColorVec4(0x1F6B46);
...
void Menu::DrawMenu()
{
    ...
    static bool m_bTeleportWindowOpen = false;
    ImGuiStyle& style = ImGui::GetStyle();
    ImGuiIO& io = ImGui::GetIO(); (void)io;
    ...
    if (menubar_window)
    {
        ...
        if (ImGui::BeginChild("body", ImVec2(window_bb.GetWidth(), ImGui::GetFrameHeight() + style.CellPadding.y * 2.0f), ImGuiChildFlags_AlwaysUseWindowPadding, ImGuiWindowFlags_NoBackground))
        {
            ...
            ImGui::SameLine();
            if (ImAdd::ButtonAccent("Teleport"))
                m_bTeleportWindowOpen = !m_bTeleportWindowOpen;
        }
        ImGui::EndChild();
        ImGui::PopStyleVar(2);
    }
            ...
            if (main_content_child)
            {
                    ...
                    if (ImAdd::BeginChild("group1", { "Enemy", "Client", "Settings" }, &enemy_client_settings_tab, ImVec2(group_width, 0.0f)))
                    {
                        if (enemy_client_settings_tab == 0)
                        {
                            ...
                            if (game::is_murder_mystery_2)
                            {
                                ...
                                {
                                    ImGui::TextWrapped("Red = Murderer, Blue = Sheriff, Gray = Innocent");
                                }
                                ImGui::Separator();
                            }
                            ...
                            // Boxes
                            if (ImGui::CollapsingHeader("Boxes & Outlines", ImGuiTreeNodeFlags_DefaultOpen))
                            {
                                ImAdd::CheckBox("Box", &settings::visuals::box);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## box color", settings::visuals::box_color);

                                ImAdd::CheckBox("Box Fill", &settings::visuals::box_fill);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## box fill color", settings::visuals::box_fill_color);
                            }

                            // Info
                            if (ImGui::CollapsingHeader("Info (Name / Distance / Tool)", ImGuiTreeNodeFlags_None))
                            {
                                ImAdd::CheckBox("Name", &settings::visuals::name);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## name color", settings::visuals::name_color);

                                ImAdd::CheckBox("Avatar", &settings::visuals::avatar);

                                ImAdd::CheckBox("Distance", &settings::visuals::distance);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## distance color", settings::visuals::distance_color);

                                ImAdd::CheckBox("Tool", &settings::visuals::tool);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## tool color", settings::visuals::tool_color);
                            }

                            // Bars & Status
                            if (ImGui::CollapsingHeader("Bars & Status", ImGuiTreeNodeFlags_None))
                            {
                                ImAdd::CheckBox("Healthbar", &settings::visuals::healthbar);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## healthbar color", settings::visuals::healthbar_color);

                                ImAdd::CheckBox("Health Percent", &settings::visuals::health_percent);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## health percent color", settings::visuals::health_percent_color);

                                ImAdd::CheckBox("Armor Bar", &settings::visuals::armorbar);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## armorbar color", settings::visuals::armorbar_color);
                            }

                            // Chams & Misc
                            if (ImGui::CollapsingHeader("Chams & Misc", ImGuiTreeNodeFlags_None))
                            {
                                ImAdd::CheckBox("Flags", &settings::visuals::flags);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## flags state color", settings::visuals::flags_state_colour);

                                ImAdd::CheckBox("Chams", &settings::visuals::chams);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## chams fill color", settings::visuals::chams_fill_color);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## chams outline", settings::visuals::chams_outline_color);
                        ...
                        {
                            if (ImGui::CollapsingHeader("Local / Client ESP", ImGuiTreeNodeFlags_DefaultOpen))
                            {
                                ImAdd::CheckBox("Enable Client", &settings::visuals::enable_client);

                                ImAdd::CheckBox("Box", &settings::visuals::client_box);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## local box color", settings::visuals::client_box_color);

                                ImAdd::CheckBox("Box Fill", &settings::visuals::client_box_fill);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## local box fill color", settings::visuals::client_box_fill_color);

                                ImAdd::CheckBox("Name", &settings::visuals::client_name);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## local name color", settings::visuals::client_name_color);

                                ImAdd::CheckBox("Avatar", &settings::visuals::client_avatar);

                                ImAdd::CheckBox("Healthbar", &settings::visuals::client_healthbar);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## local healthbar color", settings::visuals::client_healthbar_color);

                                ImAdd::CheckBox("Health Percent", &settings::visuals::client_health_percent);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## local health percent color", settings::visuals::client_health_percent_color);

                                ImAdd::CheckBox("Armor Bar", &settings::visuals::client_armorbar);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## local armorbar color", settings::visuals::client_armorbar_color);

                                ImAdd::CheckBox("Distance", &settings::visuals::client_distance);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## local distance color", settings::visuals::client_distance_color);

                                ImAdd::CheckBox("Tool", &settings::visuals::client_tool);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## local tool color", settings::visuals::client_tool_color);

                                ImAdd::CheckBox("Flags", &settings::visuals::client_flags);
                                ImGui::SameLine(); ImAdd::ColorEdit4("## local flags color", settings::visuals::client_flags_state_colour);
                            }
                        ...
                        else if (enemy_client_settings_tab == 2)
                        {
                            if (ImGui::CollapsingHeader("General Settings", ImGuiTreeNodeFlags_DefaultOpen))
                            {
                                ...
                                ImAdd::Combo("Name Display", &settings::visuals::name_display_type, { "Display Name", "Username" });
                            ...
                            if (ImGui::CollapsingHeader("Filtering & Behavior", ImGuiTreeNodeFlags_None))
                            {
                            ...
                            if (ImGui::CollapsingHeader("Radar & Overlays", ImGuiTreeNodeFlags_None))
                            {
                                ...
                                ImAdd::SliderInt("Overlay FPS Cap", &settings::visuals::overlay_fps_cap, 0, 240);
                                ImGui::TextDisabled("0 = uncapped");
                            }

                            if (ImGui::CollapsingHeader("Color Blending", ImGuiTreeNodeFlags_None))
                            {
                                ImAdd::CheckBox("Blend", &settings::visuals::blend);
                                if (settings::visuals::blend)
                                {
                                    ImAdd::ColorEdit4("Blend Start", settings::visuals::name_color_blend_start);
                                    ImAdd::ColorEdit4("Blend End", settings::visuals::name_color_blend_end);
                                }
                            }
                        }
                    }
                ...
                else if (logical_tab == 3)
                {
                    ...
                    if (ImAdd::BeginChild("Movement Settings", ImVec2(group_width, group_height)))
                    {
                        ...
                        {
                            ImAdd::SliderFloat("Gravity", &settings::movement::gravity::value, -100.0f, 400.0f, "%.1f");
                            ImGuiKey gravity_key = ImGuiKey_None;
                            gravity_key = keybind::vk_to_imgui_key(settings::movement::gravity::keybind);
                            if (ImAdd::KeyBind("## Gravity Keybind", &gravity_key, ImVec2(0,0), &settings::movement::gravity::activation_mode))
                            {
                                settings::movement::gravity::keybind = keybind::imgui_key_to_vk(gravity_key);
                            }
                        }
                    ...
                    if (ImAdd::BeginChild("Rage 2", ImVec2(big_child_width, big_child_height)))
                    {
                        ...
                        if (settings::movement::orbit::enabled)
                        {
                            ...
                            ImAdd::Combo("##Target Type", &settings::movement::orbit::orbit_type, { "Aimbot Target", "Silent Target", "Selected Player" });
                ...
                else if (logical_tab == 4)
                {
                    ...
                    if (ImAdd::BeginChild("Settings", ImVec2(group_width, ImGui::GetContentRegionAvail().y)))
                    {
                        ...
                        ImAdd::SliderInt("Overlay FPS Cap", &settings::visuals::overlay_fps_cap, 0, 240);
                        ImGui::TextDisabled("Overlay FPS cap");
                        ImAdd::CheckBox("V-Sync", &settings::menu::vsync);
                        if (ImAdd::CheckBox("Hide Console", &external_config::hide_console))
                        {
            ...
            if (ImAdd::BeginChild("pl_body", ImGui::GetContentRegionAvail() - style.WindowPadding))
            {
                ...
                if (ImAdd::BeginChild("##PL_Actions", ImVec2(right_w - 6.0f, list_h)))
                {
                    ...
                    else
                    {
                            ...
                            try {
                                ...
                                if (ri != cache::cached_local_player.parts.end() && ri->second.address)
                                {
                                    ...
                                    if (lp)
                                    {
                                        ...
                                        for (int w = 0; w < 10000; w++) { memory->write<math::vector3>(lp + Offsets::Primitive::Position, t); memory->write<math::vector3>(lp + Offsets::Primitive::AssemblyLinearVelocity, math::vector3(0, 0, 0)); }
                                    }
                                }
                            } catch (...) {}
                        }
                        ...
                        // Teleport to slowly
                        if (ImAdd::ButtonAccent("Teleport to slowly", ImVec2(ImGui::GetContentRegionAvail().x, 0)) && has_tp)
                        {
                            try {
                                // prevent spawning multiple concurrent teleports
                                static std::atomic_bool tp_in_progress{ false };
                                if (tp_in_progress.load())
                                {
                                    // already running, ignore this request
                                }
                                else
                                {
                                    tp_in_progress.store(true);

                                // Spawn a thread to interpolate position over time (~5 seconds)
                                std::lock_guard<std::mutex> lock(cache::mtx);
                                auto ri = cache::cached_local_player.parts.find("HumanoidRootPart");
                                if (ri != cache::cached_local_player.parts.end() && ri->second.address)
                                {
                                    ...
                                    if (lp)
                                    {
                                        ...
                                        math::vector3 start = settings::rage::playerlist::saved_position;
                                        // compute final target position offset by configured distance from target
                                        math::vector3 final = tp;
                                        math::vector3 dir = final - start;
                                        float dist = dir.length();
                                        if (dist > 0.0001f) {
                                            math::vector3 n = dir.normalized();
                                            final = tp - n * settings::magicbullet::offset_distance;
                                        } else {
                                            final.x += settings::magicbullet::offset_distance; // fallback offset
                                        }

                                        const int duration_ms = 5000; // 5 seconds

                                        std::thread([lp, start, final, duration_ms]() {
                                            try {
                                                ...
                                                while (true) {
                                                    ...
                                                    float tt = static_cast<float>(elapsed) / static_cast<float>(duration_ms);
                                                    if (tt < 0.f) tt = 0.f; if (tt > 1.f) tt = 1.f;
                                                    math::vector3 pos = start * (1.0f - tt) + final * tt;
                                                    try {
                                                        // a small number of writes per step to reduce load
                                                        for (int k = 0; k < 3; ++k) {
                                                            memory->write<math::vector3>(lp + Offsets::Primitive::Position, pos);
                                                            memory->write<math::vector3>(lp + Offsets::Primitive::AssemblyLinearVelocity, math::vector3(0, 0, 0));
                                                        }
                                                    } catch (...) {}
                                                    std::this_thread::sleep_for(std::chrono::milliseconds(2));
                                                }

                                                // final burst to try to stick / fling (kept small)
                                                try {
                                                    for (int k = 0; k < 200; ++k) {
                                                        memory->write<math::vector3>(lp + Offsets::Primitive::Position, final);
                                                        memory->write<math::vector3>(lp + Offsets::Primitive::AssemblyLinearVelocity, math::vector3(0, 0, 0));
                                                    }
                                                } catch(...) {}
                                            }
                            ...
                            if (ImAdd::ButtonAccent("Teleport Back", ImVec2(ImGui::GetContentRegionAvail().x, 0)))
                            {
                                try {
                                    ...
                                    if (ri != cache::cached_local_player.parts.end() && ri->second.address)
                                    {
                                        ...
                                        if (lp)
                                        {
                                            for (int w = 0; w < 10000; w++) { memory->write<math::vector3>(lp + Offsets::Primitive::Position, settings::rage::playerlist::saved_position); memory->write<math::vector3>(lp + Offsets::Primitive::AssemblyLinearVelocity, math::vector3(0, 0, 0)); }
...
// Teleport Manager Window
    if (m_bTeleportWindowOpen)
    {
        ...
        ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0));
        ImGui::PushStyleVar(ImGuiStyleVar_WindowBorderSize, 0);
        bool tp_window = ImGui::Begin("##TeleportWnd", &m_bTeleportWindowOpen, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
        ImGui::PopStyleVar(2);
        ...
        if (tp_window)
        {
            ...
            if (ImGui::GetCurrentWindow()->Flags & ImGuiWindowFlags_NoBackground)
            {
                ...
                ImAdd::RenderText(window_bb.Min + ImVec2(style.FramePadding.x + style.WindowBorderSize * 3.0f, style.FramePadding.y + style.WindowBorderSize * 4.0f), "Teleport Manager", NULL, false, true);
            }

            ImGui::SetCursorScreenPos(window_bb.Min + ImVec2(style.WindowPadding.x, ImGui::GetFrameHeight() + style.WindowBorderSize * 3.0f + 3.0f));
            ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, style.WindowPadding);
            ImGui::PushStyleVar(ImGuiStyleVar_ChildPadding, ImVec2(style.WindowPadding.x, 4.6f));
            if (ImAdd::BeginChild("tp_body", ImGui::GetContentRegionAvail() - style.WindowPadding))
            {
                static char name_buf[64] = "";
                ImGui::InputText("Name", name_buf, sizeof(name_buf));
                ImGui::Spacing();
                if (ImAdd::ButtonAccent("Save Current Location", ImVec2(ImGui::GetContentRegionAvail().x, 0)))
                {
                    try {
                        std::lock_guard<std::mutex> lock(cache::mtx);
                        auto ri = cache::cached_local_player.parts.find("HumanoidRootPart");
                        if (ri != cache::cached_local_player.parts.end() && ri->second.address)
                        {
                            uintptr_t prim = memory->read<uintptr_t>(ri->second.address + Offsets::BasePart::Primitive);
                            if (prim)
                            {
                                math::vector3 pos = memory->read<math::vector3>(prim + Offsets::Primitive::Position);
                                settings::teleport::location_entry entry;
                                entry.pos = pos;
                                if (std::strlen(name_buf) > 0) entry.name = name_buf; else entry.name = "Location " + std::to_string(settings::teleport::saved_locations.size() + 1);
                                std::lock_guard<std::mutex> tlock(settings::teleport::mtx);
                                settings::teleport::saved_locations.push_back(std::move(entry));
                            }
                        }
                    } catch (...) {}
                }
                ...
                std::lock_guard<std::mutex> tlock(settings::teleport::mtx);
                if (settings::teleport::saved_locations.empty())
                {
                    ImGui::TextDisabled("No saved locations");
                }
                else
                {
                    for (int i = 0; i < static_cast<int>(settings::teleport::saved_locations.size()); ++i)
                    {
                        auto &loc = settings::teleport::saved_locations[i];
                        ImGui::PushID(i);
                        ImGui::Text("%s", loc.name.c_str());
                        ImGui::SameLine();
                        std::string tp_label = "Teleport##" + std::to_string(i);
                        if (ImAdd::ButtonAccent(tp_label.c_str(), ImVec2(80, 0)))
                        {
                            lt2::teleport_to(loc.pos);
                        }
                        ImGui::SameLine();
                        std::string del_label = "Delete##" + std::to_string(i);
                        if (ImAdd::ButtonAccent(del_label.c_str(), ImVec2(80, 0)))
                        {
                            settings::teleport::saved_locations.erase(settings::teleport::saved_locations.begin() + i);
                            ImGui::PopID();
                            break; // container changed
                        }
                        ImGui::SameLine();
                        ImGui::Text("(%.1f, %.1f, %.1f)", loc.pos.x, loc.pos.y, loc.pos.z);
                        ImGui::PopID();
                    }
                }
